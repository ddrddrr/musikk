...
``Should Have'' requirements mostly represent features that would significantly improve the user experience,
but are not strictly needed for the application to function.
...
While the requirements in the following sections list what the system must
do, use cases show the same functionality from the User's perspective. This
makes further modelling simpler, since the minimal subset of actions any
given User can take is defined explicitly. The same use cases also define
tasks for the User Testing phase, where each task corresponds
to one action from the use case diagrams (see \chapterref{ch:user_testing_improvements}).
...
\begin{itemize}
    ...
    \item The system shall have a way to search Songs, Collections, Users, and Artists.
...
\begin{itemize}
    ...
    \item
    The system shall guard against common web threats~(CSRF, XSS, etc.) and follow defense in depth, so that stored content stays protected and misconfigured access does not expose data.
    \item
