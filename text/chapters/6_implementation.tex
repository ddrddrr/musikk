...
In the end of the section, the software tools used for these processing steps are selected.
...
HLS licencing will not be an issue, since it is free to use for non-commercial purposes.
...
encoded audio via HLS.~\parencite{apple_hls_specs_flac_container}

But before the audio is packaged, it is needed to be converted to multiple formats in order to support bitrate
switching.
...
typically to a different codec and/or different encoding settings: bitrate, sample rate, channels, etc.
...
To reduce the size, the audio is usually processes using a special program called \textit{codec},
which is a program that compresses and stores audio in a more efficient format.
...
such as Opus or MP3 , were researched; however, for higher bitrates that are used in Streaming they did not
...
CBR provides greater reliability and predictability when Streaming.
...
and provides high speeds of audio conversion, since it is written in C and is optimized to use multiple threads.

\texttt{ffmpeg} provides multiple codec implementations, out of which \texttt{libfdk\_aac} and \texttt{flac} were used.
\texttt{libfdk\_aac} was chosen because per documentation it is the best AAC codec that \texttt{ffmpeg} provides, and
only a single implementation for FLAC encoding exists.~\parencite{ffmpeg_libfdkaac,ffmpeg_flac}.
...
This is due to the problems that \texttt{ffmpeg} has with generating manifests that are
% todo link to frontend
compatible with Shaka Player~\parencite{shaka_player}~(which is used on the Frontend for audio playback, see Frontend for details)

Therefore, Shaka Packager was selected for the packaging step.~\parencite{shaka_packager}
