...
Authentication for each HTTP request involves attaching the access token as an HTTP \texit{Bearer} header.
...
This is done via a feature called \texit{axios interceptors}~\cite{axiosintercept}.
...
The response interceptor has an "on error" handler for responses with \texit{NOT AUTHORIZED} status code(401); this usually means that the token was not set on the request. Hence, it tries to obtain a new one
...
containing the device's unique identifier, then would be added to the \texit{LocalStorage}.
